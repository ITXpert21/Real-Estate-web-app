...
export default class UserService {
    ...
    static async search(term, userType, agency) {
        const {users} = await Http.get(`api/users/search?q=${term}&userType=${userType}&agency=${agency}`);
        if (!agency) {
            ...
            return map(users, user => ({
                disabled: !user.agency, value: user._id, label: `${user.name} ${user.surname} - ${user.email}`, agency: user.agency
            }));
        }
    }
